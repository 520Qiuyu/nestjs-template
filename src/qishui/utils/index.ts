...
import type { RouterData } from '@/types/qishui';
...
/**
 * 从页面 HTML 的 script 中提取汽水音乐注入的路由数据。
 *
 * @example
 * const routerData = parseRouterData(html);
 */
export const parseRouterData = (html: string) => {
  const $ = load(html);
...
const getExtensionByContainer = (container?: string) => {
  ...
  if (
    ...
    normalizedContainer.includes('m4a')
  )
    return 'm4a';
  ...
  if (
    normalizedContainer.includes('wave') ||
